...
//! assert_eq!(doc.group_status_counts[0].count, Some(454));
//!
...
pub struct StatusCount {
    ...
    /// `DtldNbOfTxs` — how many there are, when the bank stated a number this
    /// crate can read.
    ///
    /// `None` rather than a dropped row: the bank asserting that a status
    /// bucket exists is information even when its count is unreadable, and a
    /// missing row is indistinguishable from a bucket the bank never mentioned.
    pub count: Option<u64>,
    /// `DtldNbOfTxs` exactly as the bank wrote it.
    pub count_raw: Option<String>,
...
fn parse_status_counts(block: &Node) -> Vec<StatusCount> {
    block
        ...
        .filter_map(|n| {
            // The status is what the row is *for*: a row that does not name one
            // says nothing and is not reported. An unreadable *count* is a
            // different matter — the bank still told us this status bucket
            // exists, and dropping the row loses that.
            Some(StatusCount {
                status: PaymentStatus::from_code(n.text_of("DtldSts")?),
                count: n.text_of("DtldNbOfTxs").and_then(|v| v.trim().parse().ok()),
                count_raw: n.text_of("DtldNbOfTxs").map(str::to_owned),
                total_ct: n
                    .text_of("DtldCtrlSum")
...
pub struct Pain002Document {
    ...
    pub msg_id: String,
    /// `GrpHdr/CreDtTm` exactly as the bank reported it.
    ///
    /// Read [`created_at`](Self::created_at) for the typed value — this field
    /// is the text, kept verbatim so a non-conforming timestamp is still
...
impl Pain002Document {
    /// When the bank generated this status report, typed.
    ///
...
impl Pain002Document {
    ...
    ///  <CstmrPmtStsRpt><GrpHdr><MsgId>M</MsgId><CreDtTm>2026-07-21T23:59:00</CreDtTm></GrpHdr>
    ///   <OrgnlGrpInfAndSts>
...
pub fn parse_pain002(xml: &str) -> Result<Pain002Document, Pain002ParseError> {
    ...
    let created_at_raw = grp_hdr.text_of("CreDtTm").unwrap_or_default().to_owned();
...
<Document xmlns="urn:iso:std:iso:20022:tech:xsd:pain.002.001.03">
  <CstmrPmtStsRpt>
    ...
    /// The legacy DK variant is a **reject-only** report, and the fixture has
    /// to look like one.
    ///
    /// `pain.002.003.03` (DFÜ-Abkommen V2.7) restricts every status element to
    /// the single value `RJCT` and drops `StsRsnInf/Prtry` and `AddtlInf`
    /// entirely — it exists to say a submission failed, and nothing else. The
    /// other fixtures here that carry `ACTC`, `PART` or a proprietary reason
    /// sit on `pain.002.001.03`, where those are legal, because a fixture that
    /// no schema admits is evidence about nothing.
    #[test]
    fn the_legacy_dk_variant_is_recognised_and_is_reject_only() {
        let xml = r#"<?xml version="1.0" encoding="UTF-8"?>
<Document xmlns="urn:iso:std:iso:20022:tech:xsd:pain.002.003.03">
  <CstmrPmtStsRpt>
    <GrpHdr>
      <MsgId>DKPSR-2026-07-14-001</MsgId>
      <CreDtTm>2026-07-14T10:20:30</CreDtTm>
    </GrpHdr>
    <OrgnlGrpInfAndSts>
      <OrgnlMsgId>DD-2026-07-001</OrgnlMsgId>
      <OrgnlMsgNmId>pain.008.003.02</OrgnlMsgNmId>
      <GrpSts>RJCT</GrpSts>
      <StsRsnInf>
        <Rsn><Cd>AM05</Cd></Rsn>
      </StsRsnInf>
        ...
        assert_eq!(
            ...
            "the DK namespace must be detected, not normalised away"
        );
        assert_eq!(doc.group_status, Some(PaymentStatus::Rjct));
        assert!(!doc.is_fully_accepted());
        assert_eq!(doc.reason_codes(), vec![&ReasonCode::Am05]);
        assert_eq!(
    ...
    fn parse_actc_accepted() {
        ...
        assert_eq!(doc.created_at_raw, "2026-07-14T10:20:30");
        assert_eq!(doc.original_msg_id, "CT-2026-07-001");
        assert_eq!(doc.group_status, Some(PaymentStatus::Actc));
        assert!(doc.group_status.as_ref().unwrap().is_accepted());
        assert_eq!(
            doc.namespace.as_deref(),
            Some("urn:iso:std:iso:20022:tech:xsd:pain.002.001.03")
        );
        assert_eq!(
            doc.original_msg_type,
...
<Document xmlns="urn:iso:std:iso:20022:tech:xsd:pain.002.001.10">
  ...
  <CstmrPmtStsRpt>
    <GrpHdr><MsgId>M</MsgId><CreDtTm>2026-07-21T23:59:00</CreDtTm></GrpHdr>
    <OrgnlGrpInfAndSts><OrgnlMsgId>O</OrgnlMsgId><OrgnlMsgNmId>pain.008.001.08</OrgnlMsgNmId></OrgnlGrpInfAndSts>
  </CstmrPmtStsRpt>
</Document>"#;
  ...
  <CstmrPmtStsRpt>
    ...
    fn a_verification_of_payee_report_is_read_as_such() {
        ...
        let matched = doc
            ...
            .unwrap();
        assert_eq!(matched.count, Some(454));
        assert_eq!(
            doc.group_status_counts
                .iter()
                .filter_map(|c| c.count)
                .sum::<u64>(),
            462
        );
